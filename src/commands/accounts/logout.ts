...
import { ArgocdAuthenticationProvider, OutputChannelService } from '../../services';
 ...
 */
export function logout(authProvider: ArgocdAuthenticationProvider, outputChannel: OutputChannelService) {
  return async (sessionId?: string) => {
    ...
    if (!targetSessionId) {
      ...
      if (!session) {
        vscode.window.showWarningMessage('No active ArgoCD session to logout from');
        return;
      }
    ...
    if (confirm === 'Logout') {
      try {
        ...
        vscode.window.showInformationMessage('Successfully logged out from ArgoCD');
