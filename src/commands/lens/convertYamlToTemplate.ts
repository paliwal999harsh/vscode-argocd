 ...
 */
export async function convertYamlToTemplate(uri: vscode.Uri, templatesProvider?: any): Promise<void> {
  try {
    ...
    const defaultName = selectedResource.name ? `${selectedResource.name}-template` : 'my-template';
    ...
    outputChannel.info(
      `Templates provider is available. Current template count: ${templatesProvider.getAllTemplates().length}`
    );
