    ...
    try {
        ...
        if (resources.length === 0) {
            vscode.window.showInformationMessage(
                'Valid YAML (no ArgoCD resources detected)'
            );
            return;
        }
                ...
                try {
                    ...
                    if (errors.length > 0) {
                        validationErrors.push(
                            `Application "${resource.name || '(unnamed)'}":\n  ${errors.join('\n  ')}`
                        );
                    } else {
                    ...
                    validationErrors.push(
                        `Application "${resource.name || '(unnamed)'}": Failed to validate - ${error}`
                    );
                }
            } else {
        ...
        if (validationErrors.length > 0) {
            ...
            const resourceList = validResources
                .map(r => `  • ${r.kind}: ${r.name || '(unnamed)'}`)
                .join('\n');
