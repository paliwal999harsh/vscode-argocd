...
export class ValidationHelper {
  ...
  private static isValidDNSSubdomain(subdomain: string): boolean {
    return !lodash.isEmpty(subdomain) && subdomain.length <= 253 && this.DNS_SUBDOMAIN_PATTERN.test(subdomain);
  }
  ...
  private static isValidLabelName(name: string): boolean {
    return !lodash.isEmpty(name) && name.length <= 63 && this.LABEL_NAME_PATTERN.test(name);
  }
}
...
export class TooltipHelper {
  ...
  static createTableTooltip(properties: Map<string, string | undefined>): vscode.MarkdownString {
    ...
    const filteredEntries = Array.from(properties.entries()).filter(([_, value]) => !lodash.isNil(value));
...
export class YamlHelper {
  private static readonly ARGOCD_APPLICATION_API_VERSIONS = ['argoproj.io/v1alpha1'];
  private static readonly ARGOCD_APPLICATION_KINDS = ['Application', 'ApplicationSet'];
