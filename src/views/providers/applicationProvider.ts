    ...
    async getChildren(element?: ArgocdItem): Promise<ArgocdItem[]> {
        ...
        if (!element) {
            ...
            try {
                ...
                this.logInfo(`Retrieved ${applications.length} application(s) and ${applicationSets.length} application set(s)`);

                const items: ArgocdItem[] = [];

                // Add applications
                items.push(...applications.map(app => 
                    new ApplicationItem(app, this.clustersCache, this.reposCache)
                ));

                // Add application sets if any
                if (applicationSets.length > 0) {
                    items.push(...applicationSets.map(appset => 
                        new ApplicationSetItem(appset.metadata.name, appset)
                    ));
                }
            ...
            const items: ArgocdItem[] = [
                new ApplicationHealthItem(
                    app.status?.health?.status || 'Unknown',
                    app
                ),
                new ApplicationSyncItem(
