...
export class TemplatesProvider extends BaseProvider<TemplateItem> {
    ...
    constructor(private appService: AppService, private context: vscode.ExtensionContext) {
        super();
    ...
    private async loadDefaultTemplates(): Promise<void> {
        try {
            ...
            const templateFiles = [
                { file: 'directory-app.yaml', name: 'Directory Application', description: 'Application using plain directory with Kubernetes manifests' },
                { file: 'helm-app.yaml', name: 'Helm Application', description: 'Application using Helm chart with values' },
                { file: 'kustomize-app.yaml', name: 'Kustomize Application', description: 'Application using Kustomize with patches and overlays' }
            ];
    ...
    async getChildren(element?: TemplateItem): Promise<TemplateItem[]> {
        ...
        if (!element) {
            ...
            const items = applicationTemplates.map(template => new TemplateItem(
                template.name,
