...
export class ArgocdAuthenticationProvider implements vscode.AuthenticationProvider {
    ...
    async getSessions(scopes?: readonly string[], options?: vscode.AuthenticationProviderSessionOptions): Promise<vscode.AuthenticationSession[]> {
        this.outputChannel.debug(`ArgocdAuthenticationProvider: Getting sessions (scopes: ${scopes?.join(', ') || 'none'})`);
        ...
        if (scopes && scopes.length > 0) {
            const filtered = this.sessions.filter(s => 
                scopes.some(scope => s.scopes.includes(scope))
            );
    ...
    async createSession(scopes: readonly string[], options: vscode.AuthenticationProviderSessionOptions): Promise<vscode.AuthenticationSession> {
        ...
        try {
            ...
            if (config.authMethod === 'username' && config.username && config.password) {
                ...
                loginSuccess = await this.cliService.loginSSO(
                    config.serverAddress,
                    config.skipTls
                );
            }
    ...
    async refreshSessions(): Promise<void> {
        ...
        if (oldSessions.length > 0 || this.sessions.length > 0) {
            ...
            this.outputChannel.info(`ArgocdAuthenticationProvider: Sessions updated (added: ${this.sessions.length}, removed: ${oldSessions.length})`);
        }
    }
