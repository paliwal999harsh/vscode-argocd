...
export class WebviewService {
  ...
  private createWebviewPanel(
    ...
    return vscode.window.createWebviewPanel(viewType, title, vscode.ViewColumn.One, {
      ...
      localResourceRoots: [vscode.Uri.file(path.join(this.context.extensionPath, 'resources', 'webview'))],
      ...options
    });
  }
   ...
   */
  private getHtmlContent(webview: vscode.Webview, htmlFile: string, cssFile: string, jsFile: string): string {
  ...
  async showAddRepositoryForm(): Promise<void> {
    ...
    panel.webview.html = this.getHtmlContent(panel.webview, 'add-repository.html', 'forms.css', 'add-repository.js');
          ...
          case 'addRepository':
            try {
              ...
              vscode.window.showInformationMessage(
                `Repository '${message.data.name || message.data.url}' added successfully`
              );
              panel.dispose();
            } catch (error) {
  ...
  async showApplicationHistory(appName: string): Promise<void> {
    ...
    panel.webview.html = this.getHtmlContent(panel.webview, 'app-history.html', 'app-history.css', 'app-history.js');
          ...
          case 'rollback':
            try {
              ...
              if (confirm === 'Rollback') {
                ...
                vscode.window.showInformationMessage(
                  `Application '${appName}' rolled back to history ID ${message.id}`
                );
                // Reload history
