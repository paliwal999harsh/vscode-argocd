...
export class AppService {
    ...
    private async executeCommand(command: string): Promise<string> {
        ...
        if (config.authMethod === 'token' && config.apiToken) {
            return await this.cliService.executeWithAuth(
                command,
    ...
    async getApplicationSets(): Promise<any[]> {
        ...
        try {
            ...
            this.outputChannel.warn('AppService: Failed to get ApplicationSets - may not be supported in this ArgoCD version');
            return [];
        }
    }
     ...
     */
    async getApplicationHistory(appName: string): Promise<Array<{id: string, revision: string, date: string, source: string}>> {
    ...
    async getApplicationManifestAtRevision(appName: string, historyId: string): Promise<string> {
        ...
        try {
            ...
            this.outputChannel.debug(`AppService: Retrieved manifest for application '${appName}' at history ID ${historyId}`);
            return output;
        } catch (error) {
    ...
    async rollbackApplication(appName: string, historyId: string): Promise<void> {
        ...
        try {
            ...
            this.outputChannel.info(`AppService: Successfully rolled back application '${appName}' to history ID ${historyId}`);
        } catch (error) {
    ...
    private async buildGitCommand(baseCommand: string): Promise<string | null> {
        ...
        const sourceType = await vscode.window.showQuickPick(
            ['Directory', 'Helm', 'Kustomize', 'Jsonnet'],
            {
    ...
    private async addHelmValues(command: string): Promise<string> {
        ...
        if (helmValues) {
            const values = helmValues.split(',').map(v => v.trim()).filter(v => v);
    ...
    async addApplicationSet(): Promise<void> {
        vscode.window.showInformationMessage('ApplicationSet creation will be implemented in a future version. Please use ArgoCD CLI or UI for now.');
    }
}
