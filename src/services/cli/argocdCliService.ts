  ...
  async executeWithAuth(
    ...
    try {
      ...
      this.outputChannel.debug(
        `ArgoCD CLI Service: Command executed successfully, output length: ${stdout.length} characters`
      );
      return stdout;
    } catch (error) {
    ...
    try {
      ...
      if (loggedInMatch) {
        ...
        this.outputChannel.info(
          `ArgoCD CLI Service: Authentication status: ${isLoggedIn ? 'Authenticated' : 'Not authenticated'}`
        );
        return isLoggedIn;
      }
